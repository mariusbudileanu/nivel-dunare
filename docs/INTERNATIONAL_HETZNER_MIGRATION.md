...
Schedules must remain independent: DE/SK/HU/HR daily; AT manual until a permanent `DORIS_PARTNER_KEY`; BG at DST-safe 09:15/21:15 Europe/Sofia gates; RS must keep its dedicated three-hour NRT plus DST-safe daily/forecast schedules and component last-known-good state; use a standard system trust store and retain the Windows/Schannel handoff until the target Linux trust chain is demonstrably valid. Never use `verify=False`, `curl -k`, HTTP downgrade or an unofficial proxy.
