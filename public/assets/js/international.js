...
export async function loadInternationalData() {
  const [status, stations, observations, latest, forecasts, sources, geojson, unmapped, qualityIssues] = await Promise.all([
    "status.json", "stations.json", "observations.json", "latest.json", "forecasts.json", "sources.json",
  ...
  return { status, stations, stationsById, observations, observationsByStation, latest, forecasts, forecastsByStation, sources, geojson, unmapped, qualityIssues };
}
