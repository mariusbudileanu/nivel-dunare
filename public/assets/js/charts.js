import { COLORS, LEADS, PLOT_CONFIG, formatDate, formatNumber, safeFilename } from "./config.js";
...
function localizedLayout() { return { ...baseLayout, separators: getLocale() === "ro-RO" ? ",." : ".," }; }
function plotDateFormat() { return getLocale() === "ro-RO" ? "%d.%m.%Y" : "%d/%m/%Y"; }
...
export async function renderLevel(id, station, observations, forecasts, range, selectedDate) {
  ...
  if (forecast.length) {
    ...
    traces.push({
      ...
      hovertemplate: `<b>${t("target")} %{x|${plotDateFormat()}}</b><br>${t("forecast")}: %{y:.0f} cm<br>${t("horizon")}: %{customdata[0]} h<br>${t("issued")}: %{customdata[1]|${plotDateFormat()}}<br>${t("validation")}: %{customdata[2]}<br>${t("observed")}: %{customdata[3]:.0f} cm<br>${t("error")}: %{customdata[4]:+.0f} cm<extra>${t("forecasted")}</extra>`
    });
  }
...
export async function renderVariation(id, station, observations, range) {
  ...
  const traces = rows.length ? [{
    ...
    hovertemplate: `<b>%{x|${plotDateFormat()}}</b><br>${t("variation")}: %{y:+.0f} cm<br>${t("trend")}: %{customdata[1]}<br>${t("level")}: %{customdata[0]:.0f} cm<extra></extra>`
  }] : noDataTrace();
  await Plotly.react(id, traces, { ...localizedLayout(), showlegend: false, yaxis: { ...baseLayout.yaxis, title: `${t("variation")} (cm)`, zeroline: true }, xaxis: { ...baseLayout.xaxis, range: range?.from && range?.to ? [range.from, range.to] : undefined } }, PLOT_CONFIG);
...
export async function renderTemperature(id, station, observations, range) {
  ...
  const traces = rows.length ? [{ x: rows.map(row => row.measurement_datetime), y: values, type: "scatter", mode: "lines+markers", name: t("waterTemperature"), line: { color: "#e58a3d", width: 3 }, marker: { size: 7 }, hovertemplate: `<b>%{x|${plotDateFormat()}}</b><br>${t("temperature")}: %{y:.1f} °C<extra></extra>` }] : noDataTrace();
  const subtitle = values.length ? `Min ${formatNumber(Math.min(...values), 1)} °C · Max ${formatNumber(Math.max(...values), 1)} °C` : t("noData");
  await Plotly.react(id, traces, { ...localizedLayout(), annotations: [{ text: subtitle, xref: "paper", yref: "paper", x: 1, y: 1.08, showarrow: false, font: { color: COLORS.secondary, size: 11 } }], showlegend: false, yaxis: { ...baseLayout.yaxis, title: `${t("temperature")} (°C)` }, xaxis: { ...baseLayout.xaxis, range: range?.from && range?.to ? [range.from, range.to] : undefined } }, PLOT_CONFIG);
...
export async function renderScores(id, station, scores) {
  ...
  await Plotly.react(id, traces, { ...localizedLayout(), showlegend: false, xaxis: { ...baseLayout.xaxis, title: t("horizon") }, yaxis: { ...baseLayout.yaxis, title: "MAE (cm)" } }, PLOT_CONFIG);
...
export async function renderComparison(id, series, mode, range) {
  const traces = series.map(({ station, observations }, index) => {
    ...
    return { x: rows.map(row => row.measurement_datetime), y, type: "scatter", mode: "lines+markers", connectgaps: false, name: station.display_name, line: { width: 2.5 }, marker: { size: 6 }, hovertemplate: `%{x|${plotDateFormat()}}<br>%{y:+.0f} cm<extra>${station.display_name}</extra>` };
  });
  await Plotly.react(id, traces.length ? traces : noDataTrace(t("chooseOneStation")), { ...localizedLayout(), yaxis: { ...baseLayout.yaxis, title: mode === "variation" ? `${t("variation24")} (cm)` : `${t("changeFirstDay")} (cm)`, zeroline: true } }, PLOT_CONFIG);
  const slugs = series.map(item => item.station.slug).join("_") || "statii";
  register(id, { title: mode === "variation" ? t("comparisonVariationTitle") : t("comparisonDeltaTitle"), slug: slugs, kind: "comparatie", range, source: `AFDJ / ${t("internationalBetaDownload")}` });
}
...
export async function downloadChartPng(id) {
  ...
  try {
    await Plotly.downloadImage(graph, { format: "png", width: 1800, height: 1000, scale: 2, filename: `nivel_dunare_${safeFilename(meta.slug)}_${safeFilename(meta.kind)}_${meta.range?.from || "start"}_${meta.range?.to || "end"}` });
  } finally {
